...
## Recently Fixed
- **Reconnect broken — DXGI DuplicateOutput fails on 2nd+ session** ✅
  - **Root cause**: DXGI Desktop Duplication allows only ONE active `IDXGIOutputDuplication` per output per process. On reconnect, `DuplicateOutput` returned `E_INVALIDARG` (0x80070057 — "already duplicating this output") because the **old Session was still alive** when the new one called `Initialize()`. The main wait loop held a `shared_ptr<Session>` copy (`sess = current_session`), giving the old session `use_count=2`. When the HELLO callback did `old_sess = std::move(current_session); old_sess.reset()`, that only decremented to 1 — the destructor didn't run, so `IDXGIOutputDuplication` was never released. The previous "fix" (11× retry loop + Sleep(100)) masked this by waiting, but `E_INVALIDARG` is NOT transient — it persists until the COM handle is actually released.
  - **Fix (lifecycle — sole ownership)**:
    1. **Main wait loop** rewritten to NEVER hold a `shared_ptr<Session>` copy outside the lock. It checks `IsRunning()` under the lock, then moves `current_session` out only if still dead — ensuring the HELLO callback is always sole owner.
    2. **HELLO callback** logs `use_count` after `Stop()` and warns if != 1. `old_sess.reset()` now guaranteed to run the destructor (releasing DXGI duplication) before any new Session is created.
    3. **Retry loop and Sleep(100) removed** — they were never the right solution. Single `DuplicateOutput` call with exact HRESULT logging.
  - **Instrumentation**: `[ScreenCapture] DTOR: releasing duplication (handle alive/no handle)`, `[ScreenCapture] Release: releasing IDXGIOutputDuplication`, `[ScreenCapture] DuplicateOutput OK`, `[HELLO] Previous session fully destroyed.` — confirms correct sequence on reconnect.
  - Changed files: `windows/src/ScreenCapture.cpp`, `windows/src/main.cpp`.
