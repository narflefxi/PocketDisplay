...
- **Reconnect broken — DXGI DuplicateOutput fails on 2nd+ session** ✅
  - **Root cause**: DXGI Desktop Duplication allows only ONE active `IDXGIOutputDuplication` per output per process. On reconnect, `DuplicateOutput` returned `E_INVALIDARG` (0x80070057 — "already duplicating this output") because the previous session's duplication handle wasn't fully released before the new session called `Initialize()`. The old `ScreenCapture::Initialize` did not call `Release()` first, so if any stale COM pointers remained (or the OS hadn't fully reclaimed the handle), the call failed permanently with no retry.
  - **Fix (three-layer)**:
    1. `ScreenCapture::Initialize()` now calls `Release()` at the top to ensure all previous DXGI state (duplication, device, context, staging texture) is fully released before re-acquiring.
    2. `DuplicateOutput` is now wrapped in a bounded retry loop (up to 11 attempts, 200ms apart) for transient errors: `E_INVALIDARG`, `E_ACCESSDENIED`, `DXGI_ERROR_ACCESS_LOST`, `DXGI_ERROR_NOT_CURRENTLY_AVAILABLE`. This handles the brief window where the OS hasn't yet released the handle after COM Release.
    3. HELLO callback in `main.cpp` adds a 100ms sleep after `old_sess->Stop()` + `old_sess.reset()` to give the OS time to release the duplication handle before the new session starts.
  - **Diagnostic logging**: Every DXGI call in `Initialize()` now logs the exact HRESULT on failure. `DuplicateOutput` retry attempts log attempt number, HRESULT, and whether the error is transient or fatal.
