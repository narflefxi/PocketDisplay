...
- **DRM graceful hint — passive banner when protected content is detected** ✅
  ...
  - **Detection (`MainActivity.kt`)**: `getBitmap(32, 18)` on the TextureView produces a 32×18 thumbnail. A 5×5 grid (25 sample points) is checked: ALL 25 must be exactly `RGB(0,0,0)` — not near-black. After 4 consecutive all-black checks at 500 ms intervals (~2 s), the hint banner is shown. Any single non-black check clears the counter and auto-hides the banner. Conservative: requires ALL sample points to be exactly pure black, so dark-but-not-DRM content (dark themes, night-mode UIs) does not trigger.
