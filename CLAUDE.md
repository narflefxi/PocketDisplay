...
- **DRM-protected video (Netflix, Disney+, Prime Video, etc.) shows as black** — Windows blocks DXGI screen capture of DRM/HDCP-protected content at the OS level (Protected Media Path replaces the captured region with pure black). This is an OS-level restriction that affects ALL screen-capture and second-display tools, not a PocketDisplay bug. Non-DRM content (YouTube, local video, normal apps) works normally; audio still plays from PC speakers. The app shows a passive, dismissible hint banner after ~2 s of sustained pure-black to inform users without blocking the stream.
...
## Recently Fixed
- **Extended mode auto-elevates for one-time VDD install** ✅
  - **Trigger condition (exact)**: Extended mode selected AND `HasActiveVirtualDisplay()==false` AND `IsProcessElevated()==false`. Never fires when the VDD is already active (persists across runs) or when the process is already elevated — both existing flows are unchanged.
  - **Fix (`windows/src/main.cpp` HELLO callback, Extended branch)**: instead of calling `EnsureVirtualDisplayDriverForExtendedMode()` directly (which fails with a "requires Administrator" string and previously let the code fall through to mirroring the last physical monitor — an effective silent revert with no user feedback), the code now spawns the current exe elevated via `ShellExecuteExW` (`runas` verb, `nShow=SW_HIDE`, `SEE_MASK_NOCLOSEPROCESS | SEE_MASK_NO_CONSOLE`) with a new headless CLI arg `--install-vdd`. The live (non-elevated) process and its USB/WiFi session are kept alive — no relaunch/teardown. Waits up to 60s (`WaitForSingleObject`) for the helper to exit, then re-checks its exit code.
  - **Headless helper (`--install-vdd`)**: handled at the very top of `main()`, before GUI/socket init. Calls `EnsureVirtualDisplayDriverForExtendedMode()` (the real `DiInstallDriver` + `Root\MttVDD` device + virtual display creation) and exits 0/1. No GUI, no ports bound, and `FreeConsole()` is called immediately (same pattern the app already uses for double-click launches) so no cmd window flashes.
  - **On success**: falls through to the normal `EnsureVirtualDisplayDriverForExtendedMode()` call, which now finds `HasActiveVirtualDisplay()==true` and returns immediately; Extended proceeds seamlessly and the existing monitor-change/`ForceReinitialize` path picks up the new virtual display.
  - **On UAC-cancel or helper failure**: does NOT silently revert. `extend` is forced to `false` for this HELLO so `cfg.extend_mode`/`PickMonitor` correctly mirror the primary monitor, and `g_gui.statusMsg` shows *"Extended mode needs a one-time Administrator setup. Setup was cancelled — staying in Mirror mode."*
  - **Exposed** `IsProcessElevated()` (was `static` in `VirtualDisplayDriver.cpp`) via `VirtualDisplayDriver.h` so `main.cpp` can check it explicitly (no string-matching on error text).
  - Changed files: `windows/src/main.cpp`, `windows/src/VirtualDisplayDriver.h/cpp`.
...
- **DRM graceful hint — passive banner when protected content is detected** ✅
  ...
  - **Detection (`MainActivity.kt`)**: `getBitmap(32, 18)` on the TextureView produces a 32×18 thumbnail. A 5×5 grid (25 sample points) is checked: ALL 25 must be near-black — each channel (R, G, B) must be ≤ 18 (`BLACK_CHANNEL_MAX`). The threshold covers software-decoder limited-range black (~16,16,16 on `c2.android.avc.decoder` / Exynos) as well as hardware-decoder exact 0,0,0, while staying far below any real content. After 4 consecutive all-black checks at 500 ms intervals (~2 s), the hint banner is shown. Any single non-black check clears the counter and auto-hides the banner. Verified on both Exynos phone (SW decoder, RRCX907V0JY) and Qualcomm tablet (HW decoder, RR2MC00EJ0J).
  - **Hint banner (`activity_main.xml`)**: Dismissible `LinearLayout` overlay (`drmHintBanner`, `GONE` by default) anchored at the screen bottom, `#CC000000` background with brand orange `#FF4500` title, Space Grotesk font, conditional wording: *"Playing protected video? Netflix, Disney+ and similar DRM content show as black here — that's a Windows restriction, not a connection problem. Everything else streams normally."* "Got it" button dismisses for the current session. Auto-hides when non-black frames return.
  - **Non-interference**: purely passive overlay; detection uses a 32×18 bitmap sample at 2 Hz overhead; does not pause, reconfigure, or affect the decoder or stream in any way. Banner is hidden before `homePanel` in z-order so it is never visible on the home screen.
  - Changed files: `android/.../MainActivity.kt`, `android/.../res/layout/activity_main.xml`, `README.md`, `CLAUDE.md`.
...
- **Android SW decoder fallback for incompatible HW decoders (e.g. Samsung Exynos)** ✅ (commit 8de1bb1)
  ...
  - **Dedup bypass**: The `configure()` dedup check (`same SPS → skip`) fires on the fallback re-configure because the SPS is unchanged. Fixed by adding a `force: Boolean = false` parameter; the watchdog passes `force = true`; normal callers are unchanged.
  - **Session scoping**: `useSoftwareFallback` resets to `false` in `resetForReconnect()` so each new connection retries hardware (in case the device or stream changes). Within a session, the flag latches to prevent flip-flopping.
  - **No regression on working devices**: The watchdog only triggers if `decodedFrameCount == 0` after 1.5 s of feeding frames. Devices with working HW decoders (e.g. Qualcomm `OMX.qcom.video.decoder.avc`) produce frames within ~100 ms and are never affected.
  - **Verification**: Exynos phone (RRCX907V0JY) now streams correctly via `c2.android.avc.decoder` in Mirror + Extended + reconnect. Qualcomm tablet (RR2MC00EJ0J) continues to use hardware decoder with no fallback triggered.
  - **Known trade-off**: ~2s connect delay on affected phones (HW is always tried first). See Known Issues for future optimization.
  - Changed files: `android/.../VideoDecoder.kt`.
